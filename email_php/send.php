...
$email_pengirim = 'rachelgultom5@gmail.com'; // Isikan dengan email pengirim
$nama_pengirim = 'Dosen IT Del'; // Isikan dengan nama pengirim
...
$mail->Password = 'fhwtscfdtciecdzs'; // Isikan dengan Password email pengirim
$mail->Port = 465;
$mail->SMTPAuth = true;
$mail->SMTPSecure = 'ssl';
...
$mail->Subject = $subjek;
$mail->Body = $content;
$mail->AddEmbeddedImage('img/LogoDel.jpg', 'logo_del', 'LogoDel.jpg'); // Aktifkan jika ingin menampilkan gambar dalam email
if(empty($attachment)){ // Jika tanpa attachment
    ...
    if($size <= 25000000){ // Jika ukuran file <= 25 MB (25.000.000 bytes)
        ...
        $send = $mail->send();
        if($send){ // Jika Email berhasil dikirim
            echo "<h3>Dosen dengan email $email_penerima berhasil diundang !</h3><br /><a href='index.php'>Kembali ke Form</a>";
        }else{ // Jika Email gagal dikirim
            echo "<h3>Dosen dengan email $email_penerima gagal diundang.Mohon periksa koneksi internet anda karena ini memerlukan koneksi yang stabil !</h3><br /><a href='index.php'>Kembali ke Form</a>";
